...
class DirectAdminAPIClient(BaseDirectAdminAPIClient):
    ...
    def filemanager_list(
        self, path: str = None, query: str = None, limit: int = None, offset: int = None
    ):
        params = {}
        if path is not None:
            params["path"] = path
        ...
        endpoint, method = FILEMANAGER_LIST
        return self._make_request(endpoint=endpoint, method=method, params=params)
    ...
    def walk(self, path: str = None):
        ...
        offset = 0
        count = limit
        output = self.filemanager_list(path=root, limit=limit, offset=offset)
        while count == limit:
            count = 0
        ...
        for dir in dirs:
            yield from self.walk(path=f"{root}/{dir}")
